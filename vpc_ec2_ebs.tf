...
resource "aws_instance" "vivek_ec2" {
  ami           = "ami-076e3a557efe1aa9c"
  instance_type = "t2.micro"
  associate_public_ip_address = "true"
  subnet_id     = aws_subnet.vivek_Subnet.id
  tags = {
    Name = "vivek_vpc_ec2_terra"
  }
}
...
resource "aws_ebs_volume" "vivekdata" {
  availability_zone = aws_instance.vivek_ec2.availability_zone
  size              = 1
...
resource "aws_volume_attachment" "vivekdata" {
  ...
  instance_id = aws_instance.vivek_ec2.id
}
